...
int main(int argc, char * argv[]) {
    ...
    if(fork()==0)
    {
        execlp(command,"","-i",srcPath,"-acodec","pcm_s16le_planar","-ar","8000","-ac","1","-f","s16le","./12.pcm",NULL);
        perror(command);
        exit(errno);
    }
    else
    ...
    if(fork()==0)
    {
        execlp(command,"","-f","s16le","-ar","8000","-ac","1","-i","./13.pcm","-i","./12.h264","-vcodec","copy","-f","mp4",dstP.c_str(),NULL);
        perror(command);
        exit(errno);
    }
    else
